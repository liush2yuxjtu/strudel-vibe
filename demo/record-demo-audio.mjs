...
//   - demo-audio.webm        (captured real WebAudio, opus)
//   - vibe-demo-audio.mp4     (muxed: real picture + real Strudel sound)
//   - timeline-audio.json     (beat offsets + measured audio/video sync offset)
...
async function humanType(page, locator, text) {
  ...
  for (const ch of text) await page.keyboard.type(ch, { delay: 26 + Math.random() * 60 });
}
...
async function main() {
  ...
  for (const prompt of PROMPTS) {
    ...
    if (ok) { beat('play', 'pattern playing'); await shot(page, `a0${idx}-playing.png`); }
    else {
    ...
    for (let i = 0; i < 4; i++) {
      await page.mouse.move(W * (0.55 + Math.random() * 0.4), H * (0.2 + Math.random() * 0.4), { steps: 16 }).catch(() => {});
      await sleep(950);
    }
  }

  // nth(0) = "四拍鼓加八分hihat" — simplest preset, least likely to make MiniMax
  // emit a function @strudel/web@1.2.6 doesn't support (e.g. .wobble) and throw.
  const chip = page.locator('.chip').nth(0);
  if (await chip.count()) {
  ...
  // fill() reliably clears the textarea first (Meta+A is flaky headless and
  // was concatenating onto leftover code -> SyntaxError -> silent finale).
  ...
  await sleep(700);
  // belt-and-braces: also hit play so the finale definitely rings out
  await page.locator('#play').click().catch(() => {});

  for (let i = 0; i < 10; i++) {
    await page.mouse.move(W * (0.5 + Math.random() * 0.45), H * (0.15 + Math.random() * 0.5), { steps: 14 }).catch(() => {});
    await sleep(800);
  }
  beat('end', 'done');
  ...
  writeFileSync(join(OUT_DIR, 'timeline-audio.json'),
  ...
  const audioPath = join(OUT_DIR, 'demo-audio.webm');
  ...
  const outMp4 = join(OUT_DIR, 'vibe-demo-audio.mp4');
